...
{
    $paymentType = Appointment::PAYMENT_METHOD;
    $paymentMethod = null;
    ...
    } elseif ($paystackEnable) {
        $paymentMethod = Appointment::PAYSTACK_ARR;
    } elseif ($phonepeEnable) {
        $paymentMethod = Appointment::PHONEPE_ARR;
    } elseif ($manuallyEnable) {
        $paymentMethod = Appointment::MANUALLY_ARR;
    }

    // Exclude payment methods based on enabled gateways
    if ($stripeEnable && $paypalEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYSTACK, Appointment::PHONEPE, Appointment::MANUALLY]);
    } elseif ($stripeEnable && $paystackEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYPAL, Appointment::PHONEPE, Appointment::MANUALLY]);
    } elseif ($stripeEnable && $phonepeEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYPAL, Appointment::PAYSTACK, Appointment::MANUALLY]);
    } elseif ($paystackEnable && $phonepeEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::STRIPE, Appointment::PAYPAL, Appointment::MANUALLY]);
    } elseif ($paystackEnable && $paypalEnable) {
    ...
    // Exclude payment methods based on enabled gateways
    if ($stripeEnable && $paypalEnable && $paystackEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PHONEPE, Appointment::MANUALLY]);
    } elseif ($stripeEnable && $paystackEnable && $phonepeEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYPAL, Appointment::MANUALLY]);
    } elseif ($stripeEnable && $phonepeEnable && $paypalEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYSTACK, Appointment::MANUALLY]);
    } elseif ($paystackEnable && $phonepeEnable && $paypalEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::STRIPE, Appointment::MANUALLY]);
    } elseif ($manuallyEnable && $stripeEnable && $paypalEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYSTACK, Appointment::PHONEPE]);
    } elseif ($manuallyEnable && $stripeEnable && $paystackEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYPAL, Appointment::PHONEPE]);
    } elseif ($manuallyEnable && $stripeEnable && $phonepeEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYPAL, Appointment::PAYSTACK]);
    } elseif ($manuallyEnable && $paystackEnable && $phonepeEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::STRIPE, Appointment::PAYPAL]);
    }

    // Exclude payment methods based on enabled gateways
    if ($manuallyEnable && $phonepeEnable && $paystackEnable && $paypalEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::STRIPE]);
    } elseif ($manuallyEnable && $paystackEnable && $stripeEnable && $paypalEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PHONEPE]);
    } elseif ($manuallyEnable && $paypalEnable && $stripeEnable && $phonepeEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYSTACK]);
    } elseif ($manuallyEnable && $paypalEnable && $paystackEnable && $stripeEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PHONEPE]);
    } elseif ($manuallyEnable && $phonepeEnable && $stripeEnable && $paystackEnable) {
        $paymentMethod = Arr::except($paymentType, [Appointment::PAYPAL]);
    }

    // If all gateways are enabled, use the default payment method
    if ($stripeEnable && $paypalEnable && $paystackEnable && $phonepeEnable && $manuallyEnable) {
