    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#subscriptionPlanPrice').text(
                (currency === '$') ?
                '$' + (result.data.plan_amount ? parseFloat(result.data.plan_amount).toFixed(2) : 0) :
                getCurrencyAmount(result.data.plan_amount ? parseFloat(result.data.plan_amount).toFixed(2) : 0, currency)
            );
            $('#subscriptionPayableAmount').text(
                (currency === '$') ?
                '$' + (result.data.payable_amount ? parseFloat(result.data.payable_amount).toFixed(2) : 0) :
                getCurrencyAmount(result.data.payable_amount ? parseFloat(result.data.payable_amount).toFixed(2) : 0, currency)
            );
